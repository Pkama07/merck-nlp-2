...
from app.internal.GraphManager import GraphManager
...
# currently allowing all origins, change this if you want to restrict access
origins = ["*"]
...
gm = GraphManager("neo4j+ssc://39f470cd.databases.neo4j.io",
                  "neo4j", "GRQHnkLdqja2PXzjYUg4wwoCxCI3uAGPOjbe6N_K6KM")
...
def parse_pdf(file_name):
    ...
    tables = camelot.read_pdf(
        inputFile, pages="1-" + str(numOfPages), line_scale=40)
    ...
    # Extraction Method
    extraction_method = [" protein precipitation ", " liquid liquid extraction ", " solid phase extraction ",
                         " immunoprecipitation ", " solid liquid extraction "]
    # Chromatography
    chromatography = [" reversed phase ", " normal phase ", " ion exchange ", " hydrophilic interaction ",
                      " liquid chromatography "]
    # Ionization Method
    ionization_method = [" turbo ionspray ",
    ...
    extractionMethodValue = valueFindSearchList(
        extraction_method, "Extraction Method")
    chromatographyValue = valueFindSearchList(chromatography, "Chromatography")
    ionizationMethodValue = valueFindSearchList(
        ionization_method, "Ionization Method")
    polarityValue = valueFindSearchList(polarity, "Polarity")
    regressionModelValue = valueFindSearchList(
    ...
    LLOQ = None
    LLOQ = re.search(r"\(LLOQ\) for this method is (.*?) with",
                     searchString, re.I)
    ...
    calibrationRange = re.search(
        r"calibration range from (.*?) to ([^\s]+)", searchString, re.I)
    ...
    for temperatures in storage_temp:
        tempTemperature = re.search(
            '(' + temperatures + ')', searchString, re.I)
        if tempTemperature != None:
            storage_temp_value = tempTemperature.group(1)
    ...
    if re.search(r"Special Requirements", searchString, re.I):
        specialRequirements = re.search(
            r"Special Requirements(.*?)1 INSTRUMENTATION", searchString, re.I)
    if specialRequirements != None:
        headers.append("Special Requirements")
        rows.append(specialRequirements.group(1).replace(":", ""))
    ...
    mass_spec_value = None
    liquid_handling = None
    ...
    def findColumnRowValues(searchTable, searchColumns, searchRows):
        global mass_spec_value
        global liquid_handling
        rowNumberList = []
        columnNumberList = []
        ...
        for row in rowNumberList:
            for column in columnNumberList:
                if searchTable[row][0] == "Mass Spectrometer":
                    mass_spec_value = searchTable[row][column]
                elif searchTable[row][0] == "Liquid Handling":
                    liquid_handling = searchTable[row][column]
                else:
        ...
        for row in range(len(searchTable)):
            if row != 0:
                for column in range(len(searchTable[row])):
                    ...
                    if column != 0:
                        headers.append(
                            searchTable[row][0] + " " + searchColumns[column - 1])
                        rows.append(searchTable[row][column])

    counter = 0

    for table in tables:
        if "Category" in re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")) and "Mass Spectrometer" in re.sub(' +', ' ',
                                                                                                                  table.df.at[
                                                                                                                      1, 0].replace(
                                                                                                                      "\n",
                                                                                                                      "")):
            ...
            manufacturer = table.df.to_numpy()[1][1]
            rows.append(table.df.to_numpy()[1][1])
        elif "Category (Equipment)" in re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")):
            columnValues = ["Manufacturer"]
            rowValues = ["Microbalance", "Analytical Balance",
                         "Refrigerated centrifuge", "pH Meter", "Plate sealer"]
            findColumnRowValues(table.df.to_numpy(), columnValues, rowValues)
            ...
            headers.append(
                re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")))
            mixedIntermediateStandardSolution = table.df.to_numpy()
            ...
            headers.append(
                re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")))
            qcSolutionTable = table.df.to_numpy()
            rows.append(table.df.to_numpy())
            # table.df.to_csv(str(table.df.at[0,0].replace("\n", "")) + '.csv')
        elif "QC ID" in re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")):
            headers.append(
                re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")))
            matrixQCTable = table.df.to_numpy()
            rows.append(table.df.to_numpy())
            # table.df.to_csv(str(table.df.at[0,0].replace("\n", "")) + '.csv')
            ...
            columnValues = ["Settings"]
            rowValues = ["Ion Source", "Ion Mode", "Q1/Q3 Resolutions", "Scan Type", "Ionization Potential(IS)",
                         "Temperature", "Curtain Gas - N2*", "GS 1 - N2*", "GS 2 - N2* ", "CAD - N2*",
                         "MR pause between mass range", "MS settling time"]
            findColumnRowValues(table.df.to_numpy(), columnValues, rowValues)
        elif "Ions Monitored" in re.sub(' +', ' ', table.df.at[0, 0].replace("\n", "")):
            searchColumns = ["Q1 m/z", "Q3 m/z",
                             "Dwell (ms)", "DP (V)", "EP (V)", "CE (V)", "CXP (V)"]
            findLabelRowValuesButTextIsTooClose(
                table.df.to_numpy(), searchColumns)
    ...
    rows.append(re.sub(
        "Page [0-9]{2}", "", re.sub("BP-[0-9]{4}", "", names)).replace("Confidential", ""))
    ...
    nodes = []
    relationships = []
    bp_node = Node('BP', {'BP_number': bpNumber})
    special_requirement_node = Node('SpecialReqiurement', {})
    if not specialRequirements == None:
        special_requirement_node = Node(
            'SpecialRequirement', {'Requirements': specialRequirements.group()})
    compoundNode = Node("Compound", {
                        "Analyte": LNumber, "Form": "", "Molecular_Weight": "", "Watson_ID": ""})
    instrumentationNode = Node("Instrumentation", {'BP_number': bpNumber})
    massSpectrometerNode = Node("MassSpectrometer", {"Name": mass_spec_value})
    LCNode = Node("LC", {"Name": bpNumber})
    liquidHandlingNode = Node("LiquidHandling", {"Name": liquid_handling})
    manufacturerNode = Node("Manufacturer", {"Name": manufacturer})
    LCComponentNode = Node("LC_Component", {"Name": ""})
    instrumentationListNode = Node("Instrumentation", {"Instrumentation": bpNumber, "Mass_Spectrometer": mass_spec_value, "LC": "",
                                                       "Liquid_Handling": liquid_handling, "Manufacturer": manufacturer, "LCcomponent": ""})
    suppliesListNode = Node("Supplies", {})
    biologicalMatrixListNode = Node("Biological_Matrix", {})
    reagentsListNode = Node("Reagents", {})
    solutionsListNode = Node("Solutions", {})
    isPreparationListNode = Node("IS_Preparation", {})
    calculationParametersListNode = Node("Calculation_Parameters", {
                                         "Model": regressionModelValue})
    epimerNode = Node("L-number", {"Property": LNumber})
    # Section 7
    StandardPreparationNode = Node("BP_Number", {"BP": bpNumber})
    mixedIntermediateStandardSolutionNode = Node("Mixed_Intermediate_Standard_Solution", {
                                                 "Table": mixedIntermediateStandardSolution})
    stockStandardSolutionNode = Node("Stock_Standard_Solution", {
                                     "Dilutent": dilutentValue, "Storage_Temp": storage_temp_value})
    workingStandardSolutionNode = Node("Working_Standard_Solution", {
                                       "Dilutent": dilutentValue, "Storage_Temp": storage_temp_value})
    # Section 8
    qcPreparationListNode = Node("QC_Preparation", {"BP": bpNumber})
    stockQCSolutionNode = Node("Stock_QC_Solution", {
                               "Dilutent": dilutentValue, "Storage_Temp": storage_temp_value})
    workingQCSolutionNode = Node("Working_QC_Solution", {
                                 "Dilutent": dilutentValue, "Storage_Temp": storage_temp_value})
    workingQCTableNode = Node("Working_QC_Table", {"Table": qcSolutionTable})
    matrixQCTableNode = Node("Matrix_QC_Table", {"Table": matrixQCTable})
    matrixQCNode = Node("Matrix_QC", {"Table": LLOQ})
    # Section 12
    operatingParameters = Node("Operating_Parameters", {"BP_Number": bpNumber})
    UPLCParametersNode = Node(
        "UPLCParameters", {"Chromatography_Method": chromatographyValue})
    MSParametersNode = Node(
        "Ion_Source", {"Ion_Source": ionizationMethodValue, "Ion_Mode": polarityValue})
    # Section 14
    ...
    nodes.append(operatingParameters)
    nodes.append(UPLCParametersNode)
    nodes.append(MSParametersNode)
    nodes.append(calculationParametersListNode)

    relationships.append(Relationship(
        bp_node, special_requirement_node, 'HAS', {}))
    relationships.append(Relationship(bp_node, compoundNode, 'ANALYZES', {}))
    relationships.append(Relationship(instrumentationNode,
                         massSpectrometerNode, 'USES', {}))
    relationships.append(Relationship(
        manufacturerNode, massSpectrometerNode, 'SUPPLIES', {}))
    relationships.append(Relationship(instrumentationNode, LCNode, 'USES', {}))
    relationships.append(Relationship(
        manufacturerNode, LCNode, 'SUPPLIES', {}))
    relationships.append(Relationship(LCNode, LCComponentNode, "HAS_A", {}))
    relationships.append(Relationship(instrumentationNode,
                         liquidHandlingNode, "USES", {}))
    relationships.append(Relationship(
        manufacturerNode, liquidHandlingNode, "SUPPLIES", {}))
    relationships.append(Relationship(
        bp_node, instrumentationNode, "USES", {}))
    relationships.append(Relationship(bp_node, suppliesListNode, 'USES', {}))
    relationships.append(Relationship(
        bp_node, biologicalMatrixListNode, 'USES', {}))
    ...
    relationships.append(Relationship(
        bp_node, StandardPreparationNode, 'USES', {}))
    relationships.append(Relationship(
        bp_node, qcPreparationListNode, 'USES', {}))
    relationships.append(Relationship(
        bp_node, isPreparationListNode, 'USES', {}))
    relationships.append(Relationship(
        bp_node, calculationParametersListNode, 'USES', {}))
    relationships.append(Relationship(
        StandardPreparationNode, stockStandardSolutionNode, 'HAS_A', {}))
    relationships.append(Relationship(
        StandardPreparationNode, workingStandardSolutionNode, 'HAS_A', {}))
    relationships.append(Relationship(
        StandardPreparationNode, mixedIntermediateStandardSolutionNode, 'HAS_A', {}))
    relationships.append(Relationship(
        qcPreparationListNode, stockQCSolutionNode, 'HAS_A', {}))
    relationships.append(Relationship(
        qcPreparationListNode, workingQCSolutionNode, 'HAS_A', {}))
    relationships.append(Relationship(
        qcPreparationListNode, matrixQCNode, 'HAS_A', {}))
    relationships.append(Relationship(
        qcPreparationListNode, workingQCTableNode, 'HAS_A', {}))
    relationships.append(Relationship(
        qcPreparationListNode, matrixQCTableNode, 'HAS_A', {}))
    relationships.append(Relationship(operatingParameters,
                         UPLCParametersNode, 'HAS_A', {}))
    relationships.append(Relationship(
        operatingParameters, MSParametersNode, 'HAS_A', {}))

    populateDB(nodes, relationships)
...
# used to search for a specific table, obviously this is not the best way to do this
...
def populateDB(nodes, relationships):
    """
    os.chdir(cwd)
    print('populating db')
    ...
    info = df.iloc[0]
    gp = GraphManager("neo4j+s://d0dc487d.databases.neo4j.io",
                        "neo4j", "TPtTcrMAhv1fW93bF30hsZsPKw5x7XbfN8O8ayFvCok")
    ...
    for curr_node in nodes:
        gm.execute(operation="create_node", node=curr_node)
    for curr_rel in relationships:
        gm.execute(operation="create_rel", rel=curr_rel)
    return {"wefe": "wef"}


@app.get("/searchDB/{_query}+{_label}+{_property}")
def search(_query, _label, _property):
    # create the index for searching the database
    # first, we identify the node with the desired information using the fulltext search
    # next, we find the BP which that node is connected to using a variable length path query
    # then, we return the top 5 (can be anything) "touched" BP nodes to the frontend, passing any important info
    print(_query)
    print(_label)
    print(_property)
    result = gm.execute(operation="search", query=_query,
                        name=f"{_label}{_property}", label=_label, property=_property)
    print(result)
    print("done")
    return {"HEllo": "jowiejf"}
